...
def handle_clients(conn, addr):
    ...
    # Här checkar programmet även om det finns kvar någon klient. När den sista klienten lämnat servern kommer hela servern (programmet) stängas ner. 
    conn.close()
    if conn in all_clients:
        all_clients.remove(conn)
        print(f'Connection from {addr} closed')
    if not all_clients:
        print("No more clients, closing server...")
        server.close()
...
def start_server():
    ...
    while True:
        try:
            ...
            # Hanterar två olika saker, 1. att stänga ner programmet för servern när alla klienter har lämnat
            # 2. Att stänga ner programmet om något skulle gå fel
            server.close()
            break
